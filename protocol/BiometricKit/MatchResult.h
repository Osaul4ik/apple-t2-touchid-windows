...
// event body, and — 26.09.2026, corrected same day — the header's own
// `ordinal` field (bytes [16:24), u64le). NOT `sequence` (bytes [0:8)): a
// first attempt at this used `sequence` on the strength of the struct
// layout comment alone; decoding real hardware captures afterward showed
// `sequence` is 0 in every event this bridge daemon emits, while `ordinal`
// is the field that actually increases monotonically, confirmed surviving
// a full reconnect (new TCP connection, new StartMatch) on real hardware —
// see MatchResult.cpp's comment for the full account. This is the one
// field in this protocol that can prove causal ordering ("this event was
// generated before/after that one") from data the SEP itself produced,
// rather than from our software's clock or attempt count — see
// VerifyConfig::rejectOrdinalAtOrBelow for why that distinction matters.
// outEnvelopeOrdinal may be null for callers (WarmUp's identity-list
// reads, RunLinuxReadySequence) that have no use for it. Returns false if
// data is shorter than the fixed 24-byte header — the caller must treat
// that as fail-closed, never as an implicit NoMatch.
bool ParseStatusEventHeader(const std::vector<uint8_t>& data,
                             uint32_t* outEmbeddedType,
                             std::vector<uint8_t>* outEventData,
                             uint64_t* outEnvelopeOrdinal = nullptr);

...
// RemoteXPC (via ProbeServiceOnPort) before trusting it — nothing here is
// used as a substitute for that verification, only as a shortcut to which
// port to verify first.
#pragma once
#include "Adapter.h"
#include <cstdint>
...
// Reads the cached port for `endpoint`, keyed by its MAC address (the one
// part of a T2 NCM adapter that is stable across reboots/replugs, unlike
// ifIndex). Returns false — leaving *outPort untouched — if there is no
// entry, the cache file doesn't exist or can't be read, or `endpoint` has
// no MAC to key on (NcmEndpoint::hasMac == false).
bool LoadCachedPort(const NcmEndpoint& endpoint, uint16_t* outPort);

// Records `port` as the last known-good port for `endpoint`. Best-effort
// and silent on failure (no %LOCALAPPDATA%, can't create the directory or
// file, endpoint has no MAC, etc.) — caching is purely an optimization,
// never something correctness depends on, so a failed save just means the
// next run scans instead of short-circuiting.

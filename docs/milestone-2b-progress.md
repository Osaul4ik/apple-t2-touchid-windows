...
### Follow-up, 17.09.2026: wired in the already-written DiscardPendingEvents() fix
- Continued the detailed cross-check from the previous entry. Found that `Connection.cpp`'s `DiscardPendingEvents()` - added at some earlier point specifically to drop events accumulated during ResetSensor/LoadCalibration/identity warm-up before the match session starts, with a comment correctly stating \"Linux keeps these out of the match event stream\" - was never actually called anywhere (`grep -rn DiscardPendingEvents` returns only its own definition and declaration, zero call sites).
- Confirmed against the reference: every `biometric_command()`/`request_with_events()` call in `bridge-xpc-probe.py` returns its own local `events` list (VERIFIED FROM SOURCE - `reset_events`, `fdr_events`, `calibration_events`, `identities_events`, etc. are each freshly created per call, never pooled). The match session's own `events` starts empty exactly at `match_reply, events = biometric_command(sock, 4, data=match_data)` - nothing from before that line ever reaches `result[\"match_events\"]`.
- This project's `pendingEvents_`, by contrast, is one queue shared across the whole connection. Without the discard, `WaitForEvent()` hands the match-session loop whatever leftover events ResetSensor/warm-up-Cancel/LoadCalibration/both IdentityList and GlobalIdentityList reads (the 0x42->0x51->0x42->0x51 stability gate) happened to observe. Confirmed on the 16.09.2026 hardware capture: the match session's first two logged events (`80 MatchingCancelled`, unnamed `94`) were both acked during LoadCalibration's own reply-wait (reqId `17B6B82C...`, the cmd 0x20 call), well before StartMatch (reqId `70726736...`) was even sent - not real match-session events at all.
- Fix: call `conn->DiscardPendingEvents()` in `VerificationEngine::Verify()` immediately before building/sending the StartMatch command, so only events queued from that point forward - which still includes anything observed during StartMatch's own `SendBiometricCommand` reply-wait, matching the reference's own per-call `events` for cmd 4 - reach the match-session loop.
- Not expected to be the root cause by itself (both stale events were consumed instantly, well before any FingerOn), but it is a confirmed, previously-unwired divergence from the reference, and cleans up every future `--verbose` capture for further diagnosis. Next hardware run should no longer show any event before the `start match` log line.
...
## Не покрито в цьому Milestone (свідомо, за ТЗ §26 "не вирішувати ще")

Power management / resume-invalidation дизайн — лише детекція намічена
(`T2EvtDeviceD0Entry` коментар), фактична інвалідація протокольної сесії
в user-mode ще не реалізована (потребує persistent Connection lifecycle
менеджера, якого зараз немає — кожен `Verify()` створює нове з'єднання
явно, що частково знімає проблему, але не для гіпотетичного майбутнього
long-lived-connection дизайну).
